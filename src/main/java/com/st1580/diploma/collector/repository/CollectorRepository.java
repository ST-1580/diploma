...
public interface CollectorRepository {
    ...
    Map<EntityType, Map<Long, List<Long>>> collectAllNeighborsIdsByEntities(Collection<Long> ids);

    default Map<EntityType, List<? extends Link>> collectAllNeighbors(long id) {
        Map<EntityType, Map<Long, List<? extends Link>>> res = collectAllNeighborsByEntities(List.of(id));
        return res.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> entry.getValue().get(id)));
    }

    Map<EntityType, Map<Long, List<? extends Link>>> collectAllNeighborsByEntities(Collection<Long> ids);
}
